...
static void* st20p_rx_thread(void* arg) {
  ...
  while (!ctx->app->exit) {
    ...
    /* DEBUG: Print detailed frame information for first few frames */
    if (ctx->frames_received <= 5) {
      printf("ST20P RX session %d: Frame %d received\n", ctx->idx, ctx->frames_received);
      printf("  Format: %d, Size: %zu bytes\n", frame->fmt, frame->data_size);
      printf("  Width: %d, Height: %d\n", frame->width, frame->height);
      printf("  Addresses: Y=%p, U=%p, V=%p\n", frame->addr[0], frame->addr[1], frame->addr[2]);
      printf("  Linesizes: Y=%d, U=%d, V=%d\n", frame->linesize[0], frame->linesize[1], frame->linesize[2]);
    }
    ...
    /* Note: Test patterns are shown in main loop, real frames are always processed here */
    if (ctx->app->enable_display && ctx->display.initialized) {
      /* Only display real frames if NOT in test pattern mode OR test pattern phase is over */
      if (!ctx->app->enable_test_pattern || !should_show_test_pattern()) {
        display_frame_x11(&ctx->display, frame);
      }
      /* Always process X11 events to keep window responsive */
...
static int create_st20p_session(struct rx_app_context* app, struct st20p_rx_ctx* ctx, int session_idx) {
  ...
  if (app->enable_display) {
    if (init_x11_display(&ctx->display, app->width, app->height, session_idx) < 0) {
      printf("Warning: Failed to initialize X11 display for session %d\n", session_idx);
  ...
  if (manager->st20p_sessions) {
    for (int i = 0; i < manager->st20p_count; i++) {
      if (manager->st20p_sessions[i].handle) {
        ...
        cleanup_x11_display(&manager->st20p_sessions[i].display);
      }
    }
  }
