...
static void print_help(const char* prog_name) {
  ...
  printf("Options:\n");
  printf("  -p, --port <port>           Network port (default: 0000:af:00.0)\n");
  ...
  printf("  -F, --fmt <format>          Frame format (default: YUV422PLANAR10LE)\n");
...
static int parse_args(struct tx_app_context* ctx, int argc, char** argv) {
  ...
  /* Set defaults */
  strncpy(ctx->port, "0000:af:01.0", sizeof(ctx->port));
  ...
  ctx->height = 1080;
  ctx->fps = ST_FPS_P25;
  ctx->fmt = ST_FRAME_FMT_YUV422PLANAR10LE;
  ...
  while ((c = getopt_long(argc, argv, "p:s:d:u:w:h:f:F:t:2:3:T:DC:?", long_options, &option_index)) != -1) {
    switch (c) {
      case 'p':
      ...
      case 'f': {
        ...
        switch (fps_val) {
          case 25: ctx->fps = ST_FPS_P25; break;
          case 30: ctx->fps = ST_FPS_P30; break;
          case 50: ctx->fps = ST_FPS_P50; break;
          case 60: ctx->fps = ST_FPS_P60; break;
          default:
      ...
      case 'F':
        if (strcmp(optarg, "yuv422p10le") == 0)
          ...
          ctx->fmt = ST_FRAME_FMT_YUV420PLANAR8;
        else if (strcmp(optarg, "yuv422p12le") == 0)
          ctx->fmt = ST_FRAME_FMT_YUV422PLANAR12LE;
        else if (strcmp(optarg, "yuv444p10le") == 0)
          ctx->fmt = ST_FRAME_FMT_YUV444PLANAR10LE;
        else if (strcmp(optarg, "yuv444p12le") == 0)
          ctx->fmt = ST_FRAME_FMT_YUV444PLANAR12LE;
        else if (strcmp(optarg, "rgb10le") == 0)
          ctx->fmt = ST_FRAME_FMT_GBRPLANAR10LE;
        else if (strcmp(optarg, "rgb12le") == 0)
          ctx->fmt = ST_FRAME_FMT_GBRPLANAR12LE;
        else {
...
int main(int argc, char** argv) {
  ...
  printf("Test duration completed, stopping...\n");
  app.exit = true;
